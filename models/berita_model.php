<?php 
	...
	*/
	class berita_model
	{
		...
		{
			$query = $this->db->prepare("SELECT * FROM `berita` WHERE `id`=:id");
		...
		{
			$query = $this->db->prepare("INSERT INTO `berita` (title,content,foto) VALUES (?,?,?) ");
		...
		{
			$query = $this->db->prepare("UPDATE `berita` SET `title` = :title,
								`content` = :content,
								`foto`	= :foto
								WHERE `id`   =:id");
		...
		{
			$query = $this->db->prepare("DELETE FROM `berita` WHERE `id` = ?");
